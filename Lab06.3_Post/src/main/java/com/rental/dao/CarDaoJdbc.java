...
public class CarDaoJdbc implements CarService {
	...
	public Collection<RentalCar> getCarsPerLocationAndPrice(String location, double price) {
		return new JdbcTemplate(datasource).query(sql + "where location  = ? and cost  < ?",
				new Object[] { location, price }, new CarRowMapper<RentalCar>());
	}
	...
	public void add(RentalCar car) {
	   String sql =  "INSERT INTO RENTALCAR (LOCATION, CATEGORY, COST, MAKE, MODEL) VALUES (?, ?, ?, ?, ?)";
	   new JdbcTemplate(datasource).update(sql, new Object[] {car.getLocation(), car.getCategory(), car.getDailyRate(), car.getMake(), car.getModel()});
	}
}
